...
OUTLIER_RANGES = {k: tuple(v) for k, v in config["OUTLIER_RANGES"].items()}  # cut crazy values
AGG_METRICS = {k: tuple(v) for k, v in config["AGG_METRICS"].items()}        # for safety score
...
df["Temp_Rate"] = df.groupby("Device ID")["Temperature (°C)"].diff() / \
...
# 1 min resolution is enough for detecting unsafe patterns in indutrial iot
df['Minute'] = df['Timestamp_IST'].dt.floor('min')
agg_df = df.groupby(['Device ID', 'Minute']).agg(
...
agg_df[['avg_temp_smooth', 'avg_current_smooth', 'total_power_smooth']] = \
    agg_df.groupby('Device ID')[['avg_temp', 'avg_current', 'total_power']].rolling(
        ROLLING_WINDOW, min_periods=1).mean().reset_index(0, drop=True)
...
agg_df[['temp_roll_max', 'temp_roll_min']] = \
    agg_df.groupby('Device ID')['avg_temp_smooth'].rolling(
        ROLLING_WINDOW, min_periods=1).agg(['max', 'min']).reset_index(0, drop=True)

agg_df[['power_roll_max', 'power_roll_min']] = \
    agg_df.groupby('Device ID')['total_power_smooth'].rolling(
        ROLLING_WINDOW, min_periods=1).agg(['max', 'min']).reset_index(0, drop=True)
...
agg_df['safety_score'] = (agg_df['safety_score'] / sum(WEIGHTS.values())) * 100  # scale 0-100

agg_df['safety_score_smooth'] = agg_df.groupby('Device ID')['safety_score'].rolling(
    ROLLING_WINDOW, min_periods=1).mean().reset_index(0, drop=True)
